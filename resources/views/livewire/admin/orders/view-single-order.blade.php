<div>
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col">
            <h5 class="fw-500 text-black">{{ $lang->data['order_details'] ?? 'Order Details' }}</h5>
        </div>
        <div class="col-auto">
            <a href="{{ route('admin.view_orders') }}" class="btn btn-icon btn-3 btn-dark text-white mb-0">
                ...
                <div class="card-footer px-4">
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            <h6 class="mb-2 fw-500">{{ $lang->data['customer'] ?? 'Customer:' }}:</h6>
                        ...
                        <div class="mt-4 position-relative text-center">
                            <p class="text-sm fw-500 mb-2 text-secondary text-border d-inline z-index-2 bg-white px-3">
                                Empresa <a href="{{url('/')}}" class="text-dark fw-600" target="_blank">{{ getApplicationName() }}</a>
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-3">
            <div class="card mb-4">
                <div class="card-body p-4">
                    <h6 class="text-uppercase fw-500">
                                <span> {{ $lang->data['modificar_estado'] ?? 'Modificar estado de orden' }}:</span>
                    </h6>
                <div class="d-flex align-items-center">
                    <div class="dropdown w-100">
                        <button class="btn btn-s {{ getStatusClass($order->status) }} mb-0 text-white w-100" type="button"
                            id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                            {{ getOrderStatus($order->status) }}
                        </button>
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <li><a class="dropdown-item" href="#" 
                                    wire:click.prevent="changeStatus(1)">
                                    {{ $lang->data['processing'] ?? 'Processing' }}</a>
                            </li>
                            <li><a class="dropdown-item" href="#" 
                                    wire:click.prevent="changeStatus(2)">
                                    {{ $lang->data['ready_to_deliver'] ?? 'Ready To Deliver' }}</a>
                            </li>
                            <li><a class="dropdown-item" href="#" 
                                    wire:click.prevent="changeStatus(3)">
                                    {{ $lang->data['delivered'] ?? 'Delivered' }}</a>
                            </li>
                            <li><a class="dropdown-item" href="#" 
                                    wire:click.prevent="changeStatus(4)">
                                    {{ $lang->data['returned'] ?? 'Returned' }}</a>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="card-body p-3">
                    @if ($orderaddons)
                        @if (count($orderaddons) > 0)
                            <h6 class="mb-2 fw-500 mt-2">{{ $lang->data['service_addons'] ?? 'Service Addons' }}</h6>
                            <ul class="list-group">
                                <li class="list-group-item border-0 d-flex p-3 mb-2 bg-gray-100 border-radius-xs">
                    ...
                    <h6 class="mb-3 fw-500 mt-2">{{ $lang->data['payments'] ?? 'Pagos' }}</h6>
                    ...
                    <div class="row">
                        ...
                        @endif
                        <div>
                            <a href="{{ url('admin/orders/print-order/' . $order->id) }}" 
                                target="_blank"
                                type="button"
                                id="printButton"
                                class="btn btn-icon btn-dark mb-0 w-100">
                    ...
                    <!-- <br>
                    <div class="col-md-8">
                            <label class="form-label">{{ $lang->data['tipo_factura'] ?? 'Tipo de Factura:' }}</label>
                            <select class="form-select" wire:model="default_printer">
                                <option value="1">  {{ $lang->data['hoja_completa'] ?? 'Hoja Completa' }} </option>
                                <option value="2"> {{ $lang->data['ticket'] ?? 'Ticket' }} </option>
                            </select>
                    </div>
                    <div class="d-flex align-items-center justify-content-end">
                            <div>
                                <button type="submit" class="btn btn-primary ms-4"
                                    wire:click.prevent="save()">{{ $lang->data['guardar'] ?? 'Guardar' }}</button>
                            </div>
                        </div> -->
                </div>
            </div>
                ...
                <form>
                                    ...
                                    <div class="col-md-6 mb-1">
                                        ...
                                        <select class="form-select" wire:model="payment_type">
                                            ...
                                            <option class="select-box" value="1">
                                                {{ $lang->data['efectivo'] ?? 'Efectivo' }}</option>
                                            <option class="select-box" value="2">
                                                {{ $lang->data['transf'] ?? 'Transf' }}</option>
                                            <option class="select-box" value="3">
                                                {{ $lang->data['tarjeta'] ?? 'Tarjeta' }}</option>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary"
                            wire:click.prevent="addPayment">{{ $lang->data['guardar'] ?? 'Guardar' }}</button>
                    </div>
                </form>
            </div>
        </div>
